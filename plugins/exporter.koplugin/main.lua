...
local XMNoteClient = require("XMNoteClient")
...
function Exporter:init()
    ...
    self.xmnote_ip = settings.xmnote_ip or "localhost"
    self.xmnote_port = 8080
    ...
    self.xmnote_export = settings.xmnote_export or false
...
function Exporter:addToMainMenu(menu_items)
    menu_items.exporter = {
        ...
        sub_item_table = {
            {
                text = _("纸间书摘"),
                checked_func = function() return self.xmnote_export end,
                sub_item_table = {
                    {
                        text = _("设置目标设备的IP"),
                        keep_menu_open = true,
                    ...
                    } ,
                    {
                        ...
                        checked_func = function() return self.xmnote_export end,
                        callback = function()
                            self.xmnote_export = not self.xmnote_export
                            if self.xmnote_export then
                    ...
                    },
                    {
                        ...
                        callback = function()
                            UIManager:show(InfoMessage:new{
                                text = T(_([[若要将KOReader的笔记发送至纸间书摘，请确保纸间书摘的版本在v3.5.6及以上。在开始导出前您需要让运行KOReader的设备与运行纸间书摘的设备处在同一局域网中，然后在纸间书摘中打开「我的」-「书摘导入」-「通过API导入」，您可以在打开的页面底部看到设备的IP。将IP输入至「设置目标设备的IP」一栏中，即可开始执行笔记的导出。]])
...
For more information, please visit https://github.com/koreader/koreader/wiki/Highlight-export.]])
            ...
            },
            {
                ...
                callback = function()
                    ...
                    if self.joplin_export or self.readwise_export or xmnote_export then
            ...
            },
            {
                ...
                callback = function()
                    ...
                    if self.joplin_export or self.readwise_export or self.xmnote_export then
                        NetworkMgr:runWhenOnline(export_callback)
                    else
                        export_callback()
                    end
...
function Exporter:saveSettings()
    local settings = {
        ...
        xmnote_ip = self.xmnote_ip,
        xmnote_export = self.xmnote_export
    }
...
function Exporter:exportClippings(clippings)
    ...
    elseif self.xmnote_export then
        exported_stamp = "纸间书摘"
        xmnote_client = XMNoteClient:new{
            server_ip = self.xmnote_ip,
            server_port = self.xmnote_port
        }
    else
    ...
    for title, booknotes in pairs(clippings) do
        ...
        if booknotes.exported[exported_stamp] ~= true or self.txt_export or self.json_export then
            ...
            elseif self.xmnote_export then
                ok, err = pcall(self.exportBooknotesToXMNote, self, xmnote_client, title, booknotes)
            end
            -- Error reporting
...
function Exporter:exportBooknotesToXMNote(client, title, booknotes)
    client:sendBooknotes(booknotes)
end
